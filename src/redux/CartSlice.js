...
const AddToCartSlice = createSlice({
    ...
    reducers: {
        ...
        removeFromCart(state, action) {
            state.cartItems.map((cartItem) => {
                if (cartItem.id === action.payload.id) {
                    ...
                    toast.error(`${cartItem.title?.split(' ').slice(0, 2).join(' ')} removed from cart`, {
                        position: "bottom-left",
                    });
                }
                localStorage.setItem("cartItems", JSON.stringify(state.cartItems));
        ...
        clearCart(state, action) {
            state.cartItems = [];
            localStorage.setItem("cartItems", JSON.stringify(state.cartItems));
            toast.error("Cart has been cleared", { position: "bottom-left" });
        },
    },
})

export default AddToCartSlice.reducer
export const { addToCart, decreaseCart, removeFromCart, getTotals, clearCart } = AddToCartSlice.actions